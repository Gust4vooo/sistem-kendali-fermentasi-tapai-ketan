...
void setup() {
  ...
  // 2 MF input Error
  FuzzySet* errorNegatif = new FuzzySet(-5, -5, -4, 4);
  FuzzySet* errorPositif = new FuzzySet(-4, 4, 8, 8);
  
  // 2 MF input Delta Error
  FuzzySet* DE_Negatif = new FuzzySet(-5, -5, -2, 2);
  FuzzySet* DE_Positif = new FuzzySet(-2, 2, 3, 3);
