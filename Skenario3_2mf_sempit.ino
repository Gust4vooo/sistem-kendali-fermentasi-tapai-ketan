...
void setup() {
  ...
  // 2 MF input Error
  FuzzySet* errorNegatif = new FuzzySet(-5, -5, -1, 1);
  FuzzySet* errorPositif = new FuzzySet(-1, 1, 8, 8);
  
  // 2 MF input Delta Error
  FuzzySet* DE_Negatif = new FuzzySet(-5, -5, -0.5, 0.5);
  FuzzySet* DE_Positif = new FuzzySet(-0.5, 0.5, 3, 3);
